...
def save_df_in_chunks(
    ...
    estimated_rows_per_chunk = int(MAX_FILE_SIZE / avg_row_size) # Calculate the number of rows that approximately fit in the target file size
    df_row_count = len(df)
    ...
    while start_row < df_row_count:
        ...
        temp_filename = os.path.join(output_dir, f'{data_source}_part_{csv_output_count}.csv')
        temp_df.to_csv(temp_filename, index=False)
        ...
        while file_size >= MAX_FILE_SIZE and estimated_rows_per_chunk > 1:
            ...
            end_row = save_half_df(df, estimated_rows_per_chunk, df_row_count, start_row, temp_filename)
            file_size = os.path.getsize(temp_filename)
...
def get_value_columns(df: pd.DataFrame, total_column: str) -> list:
    return [col for col in df.columns if col not in ['date', 'geography', 'geography code', total_column]]
