...
if __name__ == '__main__':
    ...
    engine = create_engine(f"postgresql://doadmin:{os.getenv('WMCSRP_DB_PASS')}@{os.getenv('POSTGRES_HOST')}:25060/wmcsrp2?sslmode=require")
    dbt = dbtRunner()
    ...
    data_spec = {
        ...
        'raw__grant360.csv': preprocess.grant360_data,
    ...
    for csv_file, _ in data_spec.items():
        try:
            create_table_with_pandas(data_dir=PREPROCESSED_DATA_DIRECTORY,
                                    csv_file=csv_file,
            ...
            with psycopg2.connect(DB_CONN) as conn:
                for csv_file, _ in data_spec.items():
                    ...
                    load_csv_to_postgres(data_dir=PREPROCESSED_DATA_DIRECTORY,
                                        csv_file=csv_file,
                                        table_name=csv_file.split('.')[0],
                                        conn=conn)
        except ValueError:
